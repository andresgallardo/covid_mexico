### Script para hacer las tablas de vida
#library(LifeTable)
#library(dplyr)
#library(ggplot2)

#CARGAR DATAFRAME covid_mort_g DEL SCRIPT covid_mexico
  t<-covid_mort_g%>%select(.,c(SEXO, CVE_GEO,DEFUNCIONES,POBLACION,MUERTES,DEF_COV,E))
...
#CARGAR DATOS DEL IHME: https://covid19.healthdata.org/mexico
  ihm<-read.csv("covid_mexico/ihme.csv")
...
####COMBINAR DATAFRAMES
  e0_mexico<-cbind(e0_proyecciones,e0_covid,e0_covid_doble,e0_ihme,e0_ihme_sup ) #hay q actualizar
  e0_mexico$entidad<-estados-1
  e0_mexico <-round(e0_mexico[1:11],2)
    nombres_a<-read.csv("covid_mexico/nombres_a.csv")
    nombres_ac<-unique(nombres_a$nombres_a)
  e0_mexico<-merge(e0_mexico,nombres_a)
  ...
  write_xlsx(diferencias,"covid_mexico/diferencias.xlsx")
