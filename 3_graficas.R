...
ggplot(e0_mexico, aes(y = reorder(nombres,e0_f_proyecciones),
  ...
  geom_text(color="black", size=2, vjust=1.7,
            aes(x=e0_f_proyecciones, label=e0_f_proyecciones))+
  geom_text(color="black", size=2, vjust=1.7,
  ...
  geom_text(aes(y="Ciudad de Mexico",x=79.42, label="CONAPO"),
            color="black", size=2, vjust=-1.05 , fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=78.58, label="Bajo (SS)"),
            color="grey30", size=2, vjust=-1.05, fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=78.02, label="Medio (IHME)"),
          color="grey50", size=2, vjust=-1.05, fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=77.48, label="Alto (IHME)"),
            color="grey70", size=2, vjust=-1.05, fontface="bold")
dev.off()
...
ggplot(e0_mexico, aes(y = reorder(nombres,e0_m_proyecciones),
  ...
  scale_y_discrete(expand = expansion(mult = c(0.03, 0.03))) +
  geom_text(color="black", size=2, vjust=1.7,
            aes(x=e0_m_proyecciones, label=e0_m_proyecciones))+
  geom_text(color="black", size=2, vjust=1.7,
  ...
  geom_text(aes(y="Ciudad de Mexico",x=73.50, label="CONAPO"),
            color="black", size=2, vjust=-1.05 , fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=71.79, label="Bajo (SS)"),
            color="grey30", size=2, vjust=-1.05, fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=70.68, label="Medio (IHME)"),
            color="grey50", size=2, vjust=-1.05, fontface="bold")+
  geom_text(aes(y="Ciudad de Mexico",x=69.64, label="Alto (IHME)"),
            color="grey70", size=2, vjust=-1.05, fontface="bold")
dev.off()
